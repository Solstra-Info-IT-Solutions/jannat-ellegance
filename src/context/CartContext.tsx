...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Refresh persisted guest-cart prices when an admin starts or changes a sale.
  useEffect(() => { if (!cartIds) return; let cancelled = false; const ids = cartIds.split(','); Promise.all(ids.map((id) => fetch(`/api/products/${id}`).then((res) => res.ok ? res.json() : null))).then((responses) => { if (cancelled) return; const prices = new Map(responses.filter(Boolean).map((response) => [response.product.id, response.product.salePrice ?? response.product.price])); const updated = store.cart.map((item) => prices.has(item.id) ? { ...item, price: prices.get(item.id) } : item); if (updated.some((item, index) => item.price !== store.cart[index].price)) store.setCart(updated); }).catch(() => undefined); return () => { cancelled = true; }; }, [cartIds]);
